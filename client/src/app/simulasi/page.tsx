...
export default function Marketplace() {
  ...
  const [filteredSearchCurrentMap, setFilteredSearchCurrentMap] = useState<any>(
    []
  );
  ...
  const GPT = (data: any) => {
    ...
    fetch("http://localhost:3000/api/description", {
      method: "POST",
      body: JSON.stringify({
        prompt: `berikan saya jawaban karangan dalam 3 kalimat (tidak perlu akurat) apakah baik untuk berinvestasi properti dengan ${data.deskripsi_bisnis} dan ${data.deskripsi_pribadi} dengan harga beli ${data.harga_dasar} sampai harga jualnya ${data.harga_sekarang} dengan jangka waktu ${data.years_on_hold}`,
      }),
      headers: {
      ...
      .then((data) => {
        setResponseGPT(data.description);
      })
      .catch((error) => {
  ...
  const handleSearch = (value: string) => {
    if (filterFlow == 0) {
      ...
      const filtered = dataSimulasi.filter((item: any) =>
  ...
  const handleMaps = (current: any) => {
    console.log(assets);
    setCurrentMarket(current);
    console.log(current);
    if (assets.find((obj: any) => obj.id_properti === current.id_properti)) {
      setFlow(2);
      console.log("halo");
    } else {
      setFlow(1);
  ...
  const handleShowAssets = (current: any) => {
    setCurrentMarket(current);
    setFlow(2);
  };
  ...
  const handleSell = () => {
    const updatedAssets = assets.filter(
      (obj: any) => obj.id_properti !== currentMarker.id_properti
    );
  ...
  const handleAssetSummary = (data: any) => {
  ...
  const handleNextYear = () => {
    ...
    if (year === 2023) {
      for (let i = 0; i < dataSimulasi.length; i++) {
        dataSimulasi[i].harga_sekarang =
          dataSimulasi[i].harga_dasar * dataSimulasi[i].pengali;
      }
    } else {
      for (let i = 0; i < dataSimulasi.length; i++) {
        dataSimulasi[i].harga_sekarang =
    ...
    if (year === 2023) {
      for (let i = 0; i < assets.length; i++) {
        assets[i].harga_sekarang = assets[i].harga_dasar * assets[i].pengali;
      ...
      for (let i = 0; i < assets.length; i++) {
        ...
        variabel = variabel + assets[i].harga_sekarang / 1000000000;
        console.log(assets[i].harga_sekarang);
        console.log(assets[i].pengali);
      }
    }
              ...
              >
                ...
                {showAssets ? (
                  <div className="overflow-auto w-full h-auto aspect-[312/150] lg:aspect-[384/301] no-scrollbar">
                    {assets.map((data: any, index: number) => (
                ...
                <div className="w-full lg:w-[20vw] h-full bg-white flex flex-col items-center">
                  ...
                  {showAssetsHistory ? (
                    <div className="overflow-auto w-full h-auto aspect-[324/155] lg:aspect-[384/301] no-scrollbar">
                      {assetsHistory.map((data: any, index: number) => (
                        <div key={index} className="w-full">
                          <button
